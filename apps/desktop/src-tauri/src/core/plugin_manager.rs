...
impl PluginManager {
    ...
    pub fn install_plugin(&self, source_path: &str) -> Result<(String, String), String> {
        ...
        if !path.is_file() { return Err("來源路徑無效或不是文件".to_string()); }
        ...
        if metadata.len() > 100 * 1024 * 1024 { return Err("插件包超過 100 MB 限制".to_string()); }
        ...
        let manifest: Manifest = serde_json::from_slice(&manifest_buf).map_err(|e| format!("manifest.json 格式無效: {}", e))?;

        if manifest.kind != expected_kind {
            return Err(format!("文件擴展名 .{} 與 manifest 中的 kind {:?} 不匹配", ext, manifest.kind));
        }
    ...
    pub fn uninstall_plugin(&self, plugin_id: &str, store: &dyn PluginStateStore) -> Result<(), String> {
        if !self.plugin_exists(plugin_id) { return Err("未找到對應的插件文件".to_string()); }
...
mod tests {
    ...
    fn test_full_plugin_lifecycle_with_store_cleanup() {
        ...
        assert!(result.is_ok(), "合法插件應安裝成功: {:?}", result.err());
    ...
    fn test_security_boundaries_enforcement() {
        ...
        create_test_zip(&mismatch_zip, r#"{"id":"bad-plugin","name":"Bad","version":"1.0","kind":"MGPN","description":"x"}"#);
