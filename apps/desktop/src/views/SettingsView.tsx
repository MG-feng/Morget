...
export default function SettingsView() {
  ...
  const load = async () => {
    setLoading(true);
    try { setS(await ipc.settings.get()); } catch { setS({ language: 'zh-TW', theme: 'dark', scale: 1.0, downloadPath: '', autoUpdate: true, fontSize: 14, fpsLimit: 60, vsync: false, premiumUI: false, adaptiveFps: true, hotkeyFullscreen: 'F11', appAutoUpdate: 'notify', appUpdateChannel: 'stable', pluginAutoUpdate: 'notify', pluginUpdateChannel: 'stable', autoStart: false, delayedStart: false, closeBehavior: 'exit', showTrayIcon: true, desktopNotifications: true, soundNotifications: false, windowMode: 'window', adminMode: false, proxyMode: 'system', proxyUrl: '', networkTimeout: 30, hardwareRender: 'gpu', cpuPrerenderFrames: 3, logEnabled: true, logMaxSizeMb: 100 }); }
  ...
  const up = async (key: string, value: any) => { if (!s) return; setS({ ...s, [key]: value }); try { await ipc.settings.set({ [key]: value }); } catch {} };
  ...
  return (
    <div>
      ...
      <div className="settings-group">
        ...
        <div className="setting-row"><div className="setting-label"><span><Tip label="插件自動更新" desc="當插件發布新版本時的處理方式。" /></span></div><div className="setting-control"><select value={s.pluginAutoUpdate || 'notify'} onChange={e => up('pluginAutoUpdate', e.target.value)}><option value="notify">通知我</option><option value="auto">自動更新</option><option value="off">關閉</option></select></div></div>
        <div className="setting-row"><div className="setting-label"><span>插件更新渠道</span></div><div className="setting-control"><select value={s.pluginUpdateChannel || 'stable'} onChange={e => up('pluginUpdateChannel', e.target.value)}><option value="all">全部版本</option><option value="beta_rc">僅 Beta/RC/Stable</option><option value="stable">僅 Stable</option></select></div></div>
      </div>

      <div className="settings-section-title">行為</div>
      <div className="settings-group">
        <div className="setting-row"><div className="setting-label"><span><Tip label="開機自啟動" desc="隨 Windows 啟動自動運行。" /></span></div><div className="setting-control"><label className="toggle"><input type="checkbox" checked={!!s.autoStart} onChange={async e => { try { await ipc.settings.setAutoStart(e.target.checked); } catch {} up('autoStart', e.target.checked); }} /><span className="slider"></span></label></div></div>
        <div className="setting-row"><div className="setting-label"><span>關閉行為</span></div><div className="setting-control"><select value={s.closeBehavior || 'exit'} onChange={async e => { try { await ipc.settings.setCloseBehavior(e.target.value); } catch {} up('closeBehavior', e.target.value); }}><option value="exit">直接退出</option><option value="minimize">最小化到托盤</option></select></div></div>
      </div>

      <div className="settings-section-title">通知</div>
      <div className="settings-group">
        <div className="setting-row"><div className="setting-label"><span>系統托盤圖標</span></div><div className="setting-control"><label className="toggle"><input type="checkbox" checked={!!s.showTrayIcon} onChange={async e => { try { await ipc.settings.setTrayIcon(e.target.checked); } catch {} up('showTrayIcon', e.target.checked); }} /><span className="slider"></span></label></div></div>
        <div className="setting-row"><div className="setting-label"><span>桌面通知</span></div><div className="setting-control"><label className="toggle"><input type="checkbox" checked={!!s.desktopNotifications} onChange={e => up('desktopNotifications', e.target.checked)} /><span className="slider"></span></label><button className="btn btn-outline" style={{padding:'4px 10px',fontSize:12}} onClick={async () => { try { await ipc.settings.testNotification(); } catch {} }}>測試</button></div></div>
        <div className="setting-row"><div className="setting-label"><span>聲音通知</span></div><div className="setting-control"><label className="toggle"><input type="checkbox" checked={!!s.soundNotifications} onChange={e => up('soundNotifications', e.target.checked)} /><span className="slider"></span></label><button className="btn btn-outline" style={{padding:'4px 10px',fontSize:12}} onClick={async () => { try { await ipc.settings.testSound(); } catch {} }}>測試</button></div></div>
      </div>
      ...
      <div className="settings-group">
        <div className="setting-row"><div className="setting-label"><span>視窗模式</span></div><div className="setting-control"><select value={s.windowMode || 'window'} onChange={async e => { try { await ipc.settings.setWindowMode(e.target.value); } catch {} up('windowMode', e.target.value); }}><option value="window">窗口</option><option value="fullscreen">全屏</option></select></div></div>
      ...
      <div style={{ opacity: s.adminMode && isAdmin ? 1 : 0.35, pointerEvents: s.adminMode && isAdmin ? 'auto' : 'none' }}>
        <div className="settings-group">
          ...
          <div className="setting-row"><div className="setting-label"><span>CPU 預渲染幀</span></div><div className="setting-control"><SliderInput min={1} max={10} step={1} value={s.cpuPrerenderFrames ?? 3} unit="幀" onChange={v => up('cpuPrerenderFrames', v)} /></div></div>
        </div>
      </div>
      ...
      <div className="settings-group">
        ...
        <div className="setting-row"><div className="setting-label"><span>緩存</span><small>{cacheSize.toFixed(2)} MB</small></div><div className="setting-control"><button className="btn btn-danger" onClick={() => setModal({ title: '清理緩存', message: `確定清理 ${cacheSize.toFixed(2)} MB？`, onConfirm: async () => { const r = await ipc.settings.clearCache(); setCacheSize(0); alert(`已清理 ${r.freedMB.toFixed(2)} MB`); } })}>清理</button></div></div>
