...
// 本地開發模式：跳過 WorkOS 登錄頁，但雲端功能 (Market/Creator/Wallet) 將顯示「需要登錄」
...
function AppContent() {
  ...
  const [authState, setAuthState] = useState<AuthState | null>(AUTH_ENABLED ? null : { isLoggedIn: false });
  ...
  // 本地開發模式下，強制加載 settings 以渲染主界面
  useEffect(() => { if (!AUTH_ENABLED && !settings) ipc.settings.get().then(setSettings); }, []);
  ...
  return (
    <div className="app-container">
      ...
      <main className="main-content">
        {view === 'plugins' && (
          <div className="panel">
            <div className="panel-header"><h2>{LangHook.get('plugins.title')} ({plugins.length})</h2><button className="btn btn-primary" onClick={async () => { const res = await ipc.plugin.installViaDialog(); if (res.success) { await dialog.alert(LangHook.get('plugins.install.success', { name: res.pluginName })); loadData(); } }}>{LangHook.get('plugins.install')}</button></div>
