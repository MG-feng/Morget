...
function SettingsContent() {
  const dlg = useDlg();
  const [activeMenu, setActiveMenu] = useState<'general' | 'appearance' | 'control' | 'advanced'>('general');
  ...
  if (!settings) return <div style={{padding: 40, textAlign: 'center'}}>載入中...</div>;

  const menus = [
    { id: 'general', label: '常規' },
    { id: 'appearance', label: '外觀' },
    { id: 'control', label: '控制' },
    { id: 'advanced', label: '高級與性能' },
  ];
    ...
    <div>
      ...
      <div className="settings-layout">
        ...
        <div className="settings-sidebar">
          {menus.map(m => (
            <div key={m.id} className={`settings-nav-item ${activeMenu === m.id ? 'active' : ''}`} onClick={() => setActiveMenu(m.id as any)}>{m.label}</div>
          ))}
        </div>
